...
grid=glo_025_1800
INP_FILE=${NPB_WORKDIR}/CODE/FIX/ww3_grid.inp.${grid}
export UFSMODELDIR=/scratch2/NCEPDEV/stmp3/Neil.Barton/CODE/ufs-weather-model_run_NeilBarton-NOAA
...
if [[ ! -f ${WW3_EXEDIR}/ww3_grid ]]; then
    path_build=$(dirname ${INP_FILE} )/build_ww3
    mkdir -p ${path_build}
    cd ${path_build}
...
if [[ -f ${INP_FILE} ]]; then
  workdir=$(dirname ${INP_FILE} )/workdir 
  ...
  mv mod_def.ww3 $(dirname ${INP_FILE})/mod_def.${grid}
